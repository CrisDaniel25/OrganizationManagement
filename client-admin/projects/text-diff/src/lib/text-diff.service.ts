...
import { DiffLineResult, DiffPart, DiffTableRowResult } from './model/text-diff.model';
import { isEmpty, isNil } from './text-diff.utils';
...
export class TextDiffService {
  ...
  private formatOutput(diffs: Diff[]): DiffTableRowResult[] {
    ...
    return diffs.reduce((rows: DiffTableRowResult[], diff: Diff) => {
      ...
      let leftContent: DiffLineResult | null | undefined = null;
      let rightContent: DiffLineResult | null | undefined = null;
      let rowTemp: DiffTableRowResult | null | undefined = null;
          ...
          diffValue
            ...
            .forEach(line => {
              ...
              if (rightDiffRow) {
                ...
                if (rightDiffRow && rightDiffRow.rightContent)
          ...
          diffValue
            ...
            .forEach(line => {
              ...
              if (leftDiffRow) {
                ...
                if (leftDiffRow && leftDiffRow.leftContent)
  ...
  private countDiffs(result: DiffTableRowResult): number {
    ...
    if (result.leftContent)
      diffCount += result.rightContent ? result.rightContent?.lineDiffs.filter(diff => diff.isDiff).length : 0;
